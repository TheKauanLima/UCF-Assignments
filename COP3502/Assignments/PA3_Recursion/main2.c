...
int main(void)
{
    ...
    permute(perm, used, 0, &isFirstPrinted);
...
void printPermutArray(int* perm)
{
    for(int i = 0; i < numFans; i++)
        printf("%s\n", fList[perm[i]]->name);
}
...
// permutation
void permute(int* perm, int* used, int k, int* p)
{
    // base case if numFans is reached
    if(k == numFans)
    {
        // check if permutation is valid, print
        if (checkPerm(perm, numFans - 1))
        {
            printPermutArray(perm);
            *p = 1;
        }
    ...
    // return if a value has been printed
    if (*p)
        return;
