/**
 * The Currency class converts an amount of money from a specific
 * country into the equivalent currency of another country given 
 * the current exchange rate.
 *
 * @author Kauan Lima
 * @version 06/12/23
 */
public class CurrencyV1
{
    public static void main(String [ ] args)
    {
        //Declare and initialize local variables
        double startingUsDollars = 6500.00;		// starting US Dollars

        double reaisSpent = 6575.75;            // Brazilian Reais spent
        double realExchangeRate = 4.882894;     // 1 US dollar = 4.882894 Reais
        double dollarsSpentInBrazil = reaisSpent / realExchangeRate;      // US dollars spent in Brazil
        double dollarsAfterBrazil = startingUsDollars - dollarsSpentInBrazil;        // US dollars remaining after Brazil
        
        double yenSpent = 450750.25;            // Japanese Yen spent
        double yenExchangeRate = 139.3597;     // 1 US dollar = 139.3597 Yen
        double dollarsSpentInJapan = yenSpent / yenExchangeRate;      // US dollars spent in Japan
        double dollarsAfterJapan = dollarsAfterBrazil - dollarsSpentInJapan;        // US dollars remaining after Japan
        
        double eurosSpent = 724.99;            // Euros spent
        double euroExchangeRate = 0.928749;     // 1 US dollar = 0.928749 Euros
        double dollarsSpentInItaly = eurosSpent / euroExchangeRate;      // US dollars spent in Italy
        double dollarsAfterItaly = dollarsAfterJapan - dollarsSpentInItaly;        // US dollars remaining after Italy

        // Message to user stating purpose
        System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
        System.out.println("This program converts an amount of money");
        System.out.println("from a specific country into the equivalent");
        System.out.println("currency of another country given the current");
        System.out.println("exchange rate.");
        System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
        System.out.println();

        // Conversion
        // Starting money
        System.out.println("Starting US dollars:\t\t" + startingUsDollars + "\n");
        
        // Brazilian conversions
        System.out.println("Brazil:\n" + " Real spent:\t\t\t" + reaisSpent);
        System.out.println(" US dollars equivalent:\t\t" + dollarsSpentInBrazil);
        System.out.println(" US dollars remaining:\t\t" + dollarsAfterBrazil + "\n");

        //Japanese conversions
        System.out.println("Japan:\n" + " Yen spent:\t\t\t" + yenSpent);
        System.out.println(" US dollars equivalent:\t\t" + dollarsSpentInJapan);
        System.out.println(" US dollars remaining:\t\t" + dollarsAfterJapan + "\n");
        
        //Italian conversions
        System.out.println("Italy:\n" + " Euros spent:\t\t\t" + eurosSpent);
        System.out.println(" US dollars equivalent:\t\t" + dollarsSpentInItaly);
        System.out.println(" US dollars remaining:\t\t" + dollarsAfterItaly);
        System.out.println("==================================================");
        
        //Remaining dollars
        System.out.println("Remaining US dollars;\t\t" + dollarsAfterItaly + "\n\n");

        // Complete the code below for Souvenir Purchases
 		System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
		System.out.println("Souvenir Purchases");
		System.out.println(" (all values in US Dollars) ");
		System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");

		//Calculations for Souvenir #1
		int costItem1 = 20;  						//cost per item of first souvenir
		int budget1 = 435;   						//budget for first item
		int totalItems1 = budget1 / costItem1;		//how many items can be purchased
		int fundsRemaining1 = budget1 % costItem1;  //how much of the budget is left

		System.out.println("Item 1");
		System.out.println("   Cost per item: $" + costItem1 );
		System.out.println("   Budget: $" + budget1);
		System.out.println("   Total items purchased: " +  totalItems1);
		System.out.println("   Funds remaining: $"  +  fundsRemaining1);
     	System.out.println();

		//Calculations for Souvenir #2
		double costItem2 = 19.75;  						//cost per item of second souvenir
		int budget2 = 375;   							//budget for second item
		int totalItems2 = budget2 / (int)costItem2;		//how many items can be purchased
		double fundsRemaining2 = budget2 % costItem2;  	//how much of the budget is left

		System.out.println("Item 2");
		System.out.println("   Cost per item: $" + costItem2 );
		System.out.println("   Budget: $" + budget2);
		System.out.println("   Total items purchased: " +  totalItems2);
		System.out.println("   Funds remaining: $"  +  fundsRemaining2);

    } // end of main method
} // end of class

